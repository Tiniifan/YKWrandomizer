...
namespace YKWrandomizer.Tool
{
    public class DataReader : BinaryReader
    {
        ...
        public IEnumerable<byte> TakeWhile(Predicate<int> predicate)
        {
            while (true)
            {
                byte b = ReadByte();
                if (predicate(b) == false)
                {
                    yield break;
                }
                else
        ...
        public uint FindUInt32BetweenRange(UInt32 search, uint start, uint end)
        {
            long temp = (uint)BaseStream.Position;
            Seek(start);
        ...
        public List<uint> FindAll(int search, uint start, uint end)
        {
            long temp = (uint)BaseStream.Position;
